...
class Ref<T = any> {
  ...
  constructor(val: T, shallow: boolean = false) {
    ...
    if (val && typeof val === 'object' && shallow) {
      const reactiveVal = reactive(<T & object>val);
...
/**
 * @descriptittn 浅层 shallow
 * @param v
 * @returns
 */
const isShallow = (v: any) => {
 ...
 * @returns
 */
const isReactive = (v: any): v is boolean => {
 ...
 * @returns
 */
const isReadonly = (v: any): v is object => {
